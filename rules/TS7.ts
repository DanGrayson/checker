...
Definition Weq { ⊢ X Y Type } ⊢ Σ f:X⟶Y, Isweq[X,Y,f,CAR] Type
   ...
   := X ⟾ Y ⟾ (_,
        _ ⟾ _ ⟾ Σ_istype[
 	  	  ...
 	  	  pi1[X,Y,CDR,_,_], f ⟾ f' ⟾ Isweq[X,Y,f,CDR,_,_,f']]).

Definition Isaprop { ⊢ X Type } ⊢ ∏ x:X, ∏ y:X, Iscontr[Id[X,x,y],CAR] Type

   := X ⟾ (_,
        _ ⟾
	 ∏_istype[
 	   X, x ⟾ ∏ y:X, Iscontr [Id[X,x,y],CAR], CDR,
	   _, x ⟾ _ ⟾
	        ∏_istype[
	  	   X, y ⟾ Iscontr[
		   		Id[X,x,y], CAR], CDR,
...
Definition Isaset { ⊢ X Type } ⊢ ∏ x:X, ∏ y:X, Isaprop[Id[X,x,y],CAR] Type
