...
and eq alpha (pos,e) (pos',e') = (
      ...
      |   APPLY(O O_ev,[Phi(_,APPLY(O O_lambda,_) as f );Phi o ;LAMBDA(x ,Phi t )]),
          APPLY(O O_ev,[Phi(_,APPLY(O O_lambda,_) as f');Phi o';LAMBDA(x',Phi t')]) 
        -> (eq alpha f f' && eq alpha o o') || (eq alpha (Reduction.beta1 f o) (Reduction.beta1 f' o'))

      | APPLY(O O_ev,[Phi(_,APPLY(O O_lambda,_) as f); Phi o; LAMBDA(x,Phi t)]), e'
	-> eq alpha (Reduction.beta1 f o) (pos',e')

      | e, APPLY(O O_ev,[Phi(_,APPLY(O O_lambda,_) as f');Phi o';LAMBDA(x',Phi t')])
