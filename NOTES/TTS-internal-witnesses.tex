...
Definitional equality between expressions occurring twice in the conclusion of
an inference rule will be up to renaming of local variables, expansion of
...
$ t \syndef t' $ for definitional equality.
...
%% \item
%% Conversion is ignored when checking definitional equality.
...
%% definitionally equal to it, for then automatic typechecking of the resulting
%% expression may fail.
...
Maybe it is better to implement this at a lower level, by implementing $t \syndef t'$ so that
$ p \syndef p' $ is always true.
