...
and fillin pos env = function
	...
	match label with
	| LAMBDA _ -> raise (Error.TypingUnimplemented(pos,"binder type encountered in scan"))
	...
	| OO OO_ev -> (
		...
		| APPLY(TT TT_Pi, [t1; APPLY(LAMBDA x,[t2])])
