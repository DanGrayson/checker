...
and fillin pos env = function
    ...
    | POS(pos,e) as oe -> POS(pos, match e with
	  ...
	  | O O_lambda -> (
	      match branches with 
	      | [t; LAMBDA( x,o)] -> 
		  Helpers.make_OO_lambda (fillin pos env t)  (x, fillin pos (ts_bind (x,t) env) o)
	      | _ -> raise Error.Internal)
	  | T ( T_Pi | T_Sigma ) -> (
	      match branches with 
	      | [t; LAMBDA( x,o)] -> 
		  Helpers.make_TT_Pi (fillin pos env t)  (x, fillin pos (ts_bind (x,t) env) o)
	  ...
	  | O O_forall -> (
		  ...
		  Helpers.make_OO_forall m m' 
		    (fillin pos env n)
		    (x, fillin pos (ts_bind (x,with_pos (get_pos n) (Helpers.make_TT_El n)) env) o)
	      | _ -> raise Error.Internal)
	  | O O_ev -> (
	      match branches with 
		[f;p;LAMBDA(_,POS(_,EmptyHole _))] -> (
		  ...
		  | APPLY(T T_Pi, [t1; LAMBDA( x,t2)])
		    -> Helpers.make_OO_ev (fillin pos env f) (fillin pos env p) (x, (fillin pos (ts_bind (x,t1) env) t2))
		  | _ -> raise (Error.TypingError(get_pos f,"expected a function type")))
	      | [f; p; LAMBDA( x,t2)] ->
		  let p = fillin pos env p in
		  let t1 = tau env p in
		  Helpers.make_OO_ev (fillin pos env f) p (x, (fillin pos (ts_bind (x,t1) env) t2))
