...
let rec tsubstfresh (subs:oSubs) (v,t) = 
  let v' = fresh v 
  in let subs' = (strip_pos v, with_pos_of v (O_variable v')) :: subs
  in (with_pos_of v v', tsubst subs' t)
and t2substfresh (subs:oSubs) (v,w,t) = 
  let v' = fresh v and w' = fresh w 
  in let subs' = (strip_pos w, with_pos_of w (O_variable w')) :: (strip_pos v, with_pos_of v (O_variable v')) :: subs 
...
and tsubst subs (pos, t) = with_pos pos (
  ...
  | T_Id (t,x,y) -> T_Id (tsubst subs t,osubst subs x,osubst subs y)
  | T_def (d,u,t,c) -> raise NotImplemented
  | T_nat -> t)
and osubst (subs:oSubs) (pos,o) = with_pos pos (
  match o with
    O_variable v -> (try strip_pos(List.assoc v subs) with Not_found -> o)
  | O_emptyHole -> o
  | O_numberedEmptyHole _ -> o
  | O_numeral _ -> o 
  | O_u _ -> o
  | O_j _ -> o
  ...
  | O_sum (tT,tT',s,s',o,(x,tS)) -> O_sum (tsubst subs tT,tsubst subs tT',osubst subs s,osubst subs s',osubst subs o,tsubstfresh subs (x,tS))
  | O_empty -> o
